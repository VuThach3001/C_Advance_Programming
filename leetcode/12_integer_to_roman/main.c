...
{
    static const int values[] = {1000, 900, 500, 400, 100, 90, 50,
                                 40,   10,  9,   5,   4,   1};
    static const char *symbols[] = {"M",  "CM", "D",  "CD", "C",  "XC", "L",
                                    "XL", "X",  "IX", "V",  "IV", "I"};
    ...
    for (int i = 0; i < (int)(sizeof(values) / sizeof(values[0])); i++)
    {
        while (num >= values[i])
        {
            ...
            memcpy(result + pos, symbols[i], len);
            pos += (int)len;
            num -= values[i];
        }
    }
...
int main(void)
{
    int nums[] = {3749, 58, 1994};

    for (int i = 0; i < (int)(sizeof(nums) / sizeof(nums[0])); i++)
    {
        char *roman = intToRoman(nums[i]);
        if (roman == NULL)
        {
