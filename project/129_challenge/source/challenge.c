...
- You need to dynamically load the symbols from the shared object library
(lib_stringfunctions.so) that you created in the last challenge
    - Create another program that uses the dlopn and dlsym functions
    - basically, convert your driver program (main.c) from the last challenge to
    dynamically load all the string functions in the shared object
*/
...
int main (void)
{
    ...
    handle = dlopen ("../lib/libStringFunctions.so", RTLD_LAZY);
